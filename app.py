...
# --- 5. THE MAIN DASHBOARD (Only visible if logged in) ---
    ...
    with st.form("client_data_form"):
        ...
        # Pre-filling the franchise details based on the database login
        ...
        submitted = st.form_submit_button(f"Generate Report (Deducts 1 Credit)")

    # --- 6. CLOUD ENGINE & LEDGER UPDATE ---
    if submitted:
        target_excel_file = file_router[selected_stream]
        st.info(f"Connecting to Master Vault...")
        ...
        try:
            ...
            if "access_token" in result:
                ...
                if file_id:
                    ...
                    st.info(f"Processing PDF Layout...")
                    time.sleep(5) 
                    ...
                    if pdf_response.status_code == 200:
                        ...
                        try:
                            ...
                            cell = ws_credits.find(st.session_state.access_code)
                            new_credit_val = st.session_state.credits - 1
                            ws_credits.update_cell(cell.row, 3, new_credit_val)
                            
                            # Update the UI session state so they don't have to refresh
