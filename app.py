...
# --- 2. THE PATTERN MAPPING (Translates names to Excel codes) ---
pattern_map = {
...
with st.form("client_data_form"):
    ...
    st.subheader("Assign Fingerprint Patterns")
    finger_labels = [
    ...
    for i, label in enumerate(finger_labels):
        with col1 if i < 5 else col2:
            selection = st.selectbox(f"{label}", list(pattern_map.keys()), key=f"f_{i}")
            finger_codes[label] = pattern_map[selection]

    submitted = st.form_submit_button("Generate Official Report")

# --- 4. CLOUD ENGINE (Silent Injection) ---
if submitted:
    ...
    client_data = {
        ...
        'D32': [[finger_codes["Left Thumb"]]], 'D33': [[finger_codes["Left Index"]]],
        'D34': [[finger_codes["Left Middle"]]], 'D35': [[finger_codes["Left Ring"]]],
        'D36': [[finger_codes["Left Little"]]], 'D37': [[finger_codes["Right Thumb"]]],
        'D38': [[finger_codes["Right Index"]]], 'D39': [[finger_codes["Right Middle"]]],
        'D40': [[finger_codes["Right Ring"]]], 'D41': [[finger_codes["Right Little"]]],
    }
